...
| DataCategory | id, project_id, key, description, revision, status | The Owner or Tech Lead manages a project-specific registry. There is no mandatory universal list. Retired or unknown IDs cannot authorize new transfers. |
| SourceLabel | id, project_id, source_ref, category_ids, revision, classified_by, classified_at, input_label_refs | An approved label identifies one source or payload object. Derived artifacts may inherit the union of classified input categories with source refs; other changes need an Owner or Tech Lead and create a revision. Unclassified objects cannot leave the local boundary. |
...
| TransferDecision | id, project_id, run_id, stage_id, provider_id, purpose, source_refs, data_categories, classification_revisions, task_scope_ref, policy_revision, decision, reason, occurred_at, request_id, coarse_size | Records allow or deny before dispatch. It stores no transferred content or credentials. |
